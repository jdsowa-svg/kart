...
Here **Q/E** mirror that: press hops; **hold Q or E + turn** enters powerslide (much lower grip than auto high-speed drift alone). Hop-on-press still gives a short sharper yaw bite with steer. **Counter-steer** (opposite turn while holding Q/E) keeps the drift-side lean and reins the slide in without breaking the powerslide — classic SMK hold-R + counter-steer. Releasing L/R eases out over a short **exit taper** (~0.3 s) from residual slip + grip recovery instead of snapping to full grip.
